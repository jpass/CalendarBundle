...
class Calendar implements CalendarInterface
{
    ...
    const DEFAULT_HOUR_MODEL = '\TFox\CalendarBundle\Service\WidgetService\Hour';
    ...
    protected $hourModel = self::DEFAULT_HOUR_MODEL;
    ...
    public function generateYear($year)
    {
        ...
        $firstYearDate = Carbon::createFromFormat('d.m.Y H:i:s', sprintf('01.01.%s 00:00:00', $year));
    ...
    public function generateWeek($date)
    {
        ...
        $dateIterator = $monday->copy();
        for ($dayi = 1; $dayi <= 7; $dayi++)
        {
            $currentDate = $dateIterator->copy();
            $day = new $this->dayModel($currentDate);
            $this->addDay($day);
            for($houri = 0; $houri <= 23; $houri++)
            {
                $currentDate = $dateIterator->copy();
                $hour = new $this->hourModel($currentDate);
                $dateIterator->addHour();
                $day->addHour($hour);
            }
        }
    }
